...
PermissionSchema.statics.findOrCreate = function(params) {
  var that = this;
  ...
  return new Promise(function (resolve, reject) {
    if (Array.isArray(params)) {
      ...
      findOrCreateOne(params, function (err, permissions) {
        if (err) return reject(err);
        resolve(permissions);
      });
    }
...
function init(rolesAndPermissions, done) {
  ...
  for (var name in rolesAndPermissions) {
    ...
    role.save(function(err, role) {
      ...
      Permission.findOrCreate(rolesAndPermissions[role.name])
